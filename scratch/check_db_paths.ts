...
async function check() {
  const { data, error } = await supabase.from('empresas').select('nome_fantasia, logo_url')
  console.log(JSON.stringify(data.filter(e => e.logo_url && e.logo_url.includes('storage')), null, 2))
}
